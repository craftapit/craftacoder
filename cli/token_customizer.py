...
def patched_calculate_and_show_tokens_and_cost(self, messages, completion=None):
    ...
    # Get token information from the router API using existing configuration
    router_url = os.environ.get("CRAFTACODER_ROUTER_URL")
    router_api_key = os.environ.get("CRAFTACODER_ROUTER_API_KEY")
    ...
    self.usage_report = f"{tokens_report}\n{subscription_report}"
...
def setup_token_customization():
    """Apply the monkey patches to customize token reporting."""
    logger.debug("Setting up token usage customization")
    aider.coders.base_coder.Coder.calculate_and_show_tokens_and_cost = patched_calculate_and_show_tokens_and_cost
    aider.coders.base_coder.Coder.show_usage_report = patched_show_usage_report
    logger.debug("Token usage customization applied")
